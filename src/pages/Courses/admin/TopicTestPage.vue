...
<template>
  <UMain class="flex flex-1 flex-col w-full h-full min-h-0 gap-4">
    ...
    <div class="flex flex-col gap-1">
      <p class="text-sm text-muted">Курс: {{ store.current.value?.title || '—' }} · {{ topicTitle }}</p>
      <h1 class="text-2xl font-medium text-highlighted">Тест темы</h1>
    </div>
    ...
    <template v-else>
      <UFormField label="Название теста">
        <UInput v-model="form.title" size="lg" class="w-full max-w-xl" placeholder="Например: Проверка по теме" />
      </UFormField>
